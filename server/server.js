...
app.get("/applications", async (req, res) => {
  if (!cookieTest(req)) {
    return res
      .status(405)
      .json({ message: "no cookie returned User is not logged in" });
  }
  try {
    // Proceed with your route logic here
    const userid = await validateCookie(req.cookies.jwt);
    if (userid === false) {
      console.log("invalid cookie was recieved");
      return res.status(405).json({
        message:
          "User is not logged in no user for that cookie or invalid cookie",
      });
    }

    const sql = `SELECT  date, title , company , location , offer, id FROM applications WHERE userid = ${userid}`;
    ...
    console.log("Invalid cookie was received:", error);
    return res.status(405).json({
      message:
        "User is not logged in, no user for that cookie or invalid cookie",
    });
...
const validateCookie = function (cookie) {
  return new Promise((resolve, reject) => {
    if (!cookie) {
      console.log("null cookie error");
      return reject("No cookie provided");
    }
    const sql =
      "SELECT username, password, unique_id FROM users WHERE login_token = ?";
    db.query(sql, [cookie], (err, result) => {
      ...
      try {
        const userid = jwt.verify(cookie, SECRET_KEY);
        if (
          result[0].username === userid.username &&
          result[0].password === userid.password
        ) {
          console.log("valid cookie");
          return resolve(result[0].unique_id);
        } else {
          console.log("cookie information did not match");
          return reject("Cookie information mismatch");
        }
      } catch (e) {
        console.log("JWT verification failed:", e);
        return reject("JWT verification failed");
      }
    });
  });
};
